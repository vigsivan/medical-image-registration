...
def triplet_gen(
    ...
    if add_feat_axis:
        ...
        reshape_dti = partial(repeat, pattern = "t d h w -> t d h w f", f=1)
    else:
        reshape = partial(repeat, pattern="d h w -> b d h w", b=1)
        reshape_dti = lambda x: x
    
    if batch_size > 1:
        raise ValueError("Currently do not support batch size greater than 1")
    
    cop = tio.CropOrPad((32,128,128))
    # cop_dti = tio.CropOrPad((128,128,128,6))
    ...
    while True:
        ...
        for i in indices:
            ...
            anat, dwi, dti = [cop(j) for j in (anat, dwi, dti)]
            # dti = cop_dti(dti)

            anat, dwi = [reshape(j.data.squeeze().numpy()) for j in (anat, dwi)]
            dti = reshape_dti(dti.data.squeeze().numpy())
...
def get_model_optimizers_losses(
    ...
    losses = [image_loss_func, Grad('l2', loss_mult=1).loss]
    weights = [1, grad_loss_weight]
...
def dti_gen(anat_path: Path, dwi_path: Path, dti_path: Path):
    anat_files = list(anat_path.iterdir())
    while True:
        np.random.shuffle(anat_files)

        for anf in anat_files:
            dwf = dwi_path/anf.name
            dtf = dti_path/(anf.name.split('.')[0]+'.npy')

            an_torch = tio.ScalarImage(anf).data
            dw_torch = tio.ScalarImage(dwf).data
            dt_torch = torch.from_numpy(np.load(dtf))

        breakpoint()
        an_torch = einops.repeat(an_torch, 'd h w -> b d h w', b=1, w=1)
        dw_torch = einops.repeat(dw_torch, 'd h w -> b d h w', b=1, w=1)

        return an_torch, dw_torch, dt_torch
